									...
									<tbody>
									    <?php $i = 1; foreach($status as $user){ ?>
									        <tr>
									            ...
									            <td><?= $user['address'] ?></td>
									            <td><a href="<?= site_url() ?>accountant/editAccountant/<?= $user['id'] ?>"><i class="fa fa-pencil"></i></a> | <a  onclick="deleteConff('<?= site_url('accountant/deleteAccountant/'.$user['id']."/same") ?>')"><i class="fa fa-trash" style="color: red;"></i></a></td>
									        </tr>
									    <?php } ?>
									</tbody>
								</table>
