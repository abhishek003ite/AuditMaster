					...
					<div class="m-content">
						...
						<div class="m-portlet m-portlet--mobile">
							...
							<div class="m-portlet__body">
								...
								<!--end: Search Form -->
		<!--begin: Datatable -->
								<table  class="display table table-hover table-responsive" id="html_table" width="100%">
									<thead>
										<tr>
											<th title="Field #1">
												S. No.
									...
									<tbody>
									    <?php $i = 1; foreach($users as $user){ ?>
									        <tr>
									            ...
									            <td><?= date('d-m-Y', strtotime($user['engagementDate'])) ?></td>
									            <td><?= $user['nameOfSMSF'] ?></td>
									            <td><?= $user['ABN'] ?></td>
									            <td><?= $user['ATTN'] ?></td>
									            <td><?= $user['nameOfTrustee1'] ?></td>
									            <td><?= $user['nameOfTrustee2'] ?></td>
									            <td><?= $user['nameOfTrustee3'] ?></td>
									            <td><?= $user['nameOfTrustee4'] ?></td>
									            <td><?= $user['nameOfTrustee5'] ?></td>
									            <td><?= $user['nameOfTrustee6'] ?></td>
									            <td><?= $user['nameOfTrCo'] ?></td>
									            <td><?= $user['ACN'] ?></td>
									            <td><?= $user['forTheYearEnded'] ?></td>
									            <td><?= $user['accountantName'] ?></td>
									            <td><?= $user['accountantAddress'] ?></td>
									            <td><?= $user['invoiceAmountGross'] ?></td>
									            <td><?= $user['invoiceGST'] ?></td>
									            <td><?= $user['otherExpASICSearch'] ?></td>
									            <td><?= $user['otherExpTitleSearch'] ?></td>
									            <td><?= $user['otherExpOtherSpecify'] ?></td>
									            <td><?= $user['invoiceTotal'] ?></td>
									            <td><?= $user['previousAuditorName'] ?></td>
									            <td><?= $user['previousAuditorCompany'] ?></td>
									            <td><?= $user['previousAuditorEmail'] ?></td>
									            <td><?= $user['auditorNotes'] ?></td>
									            <td><a href="<?= site_url() ?>auditMaster/editAudit/<?= $user['id'] ?>"><i class="fa fa-pencil"></i></a> | <a  onclick="deleteConff('<?= site_url('auditMaster/deleteAudit/'.$user['id']."/same") ?>')"><i class="fa fa-trash" style="color: red;"></i></a></td>
									        </tr>
									    <?php } ?>
									</tbody>
								</table>
