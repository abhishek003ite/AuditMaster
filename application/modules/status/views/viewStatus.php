									...
									<tbody>
									    <?php $i = 1; foreach($status as $user){ ?>
									        <tr>
									            <td><?= $i ?></td>
									            <td><?= $user['status'] ?></td>
									            <td><a href="<?= site_url() ?>status/editStatus/<?= $user['id'] ?>"><i class="fa fa-pencil"></i></a> | <a  onclick="deleteConff('<?= site_url('status/deleteStatus/'.$user['id']."/same") ?>')"><i class="fa fa-trash" style="color: red;"></i></a></td>
									        </tr>
									    <?php } ?>
									</tbody>
								</table>
