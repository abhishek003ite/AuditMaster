									...
									<tbody>
									    <?php $i = 1; foreach($users as $user){ ?>
									        <tr>
									            ...
									            <td><?= $user['mobile'] ?></td>
									            <td><?= $user['role'] ?></td>
									            <td><?php if(!empty($user['profilePic'])){ ?><img src="<?= base_url('assets/app/media/img/users/').$user['profilePic'] ?>" style="width: 100px;"><?php } ?></td>
									            <td><?php if($user['isEnabled'] == 1){ ?><span class="m-badge m-badge--success m-badge--wide">Active</span><?php }else{ ?><span class="m-badge m-badge--danger m-badge--wide">Suspended</span><?php } ?></td>
									            <td><a href="<?= site_url() ?>users/editUser/<?= $user['userId'] ?>"><i class="fa fa-pencil"></i></a> | <a  onclick="deleteConff('<?= site_url('users/deleteUser/'.$user['userId']."/same") ?>')"><i class="fa fa-trash" style="color: red;"></i></a></td>
									        </tr>
									    <?php } ?>
									</tbody>
								</table>
